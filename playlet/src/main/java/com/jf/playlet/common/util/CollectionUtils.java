...
public class CollectionUtils {
    ...
    public static <T, U> Page<U> convertPage(Page<T> from, Function<T, U> func) {
        ...
        Page<U> page = new Page<>(from.getCurrent(), from.getSize(), from.getTotal());
        page.setRecords(from.getRecords().stream().map(func).filter(Objects::nonNull).collect(Collectors.toList()));
        return page;
    }
     ...
     * @param from         源分页对象
     * @param func         转换函数
     * @param afterConvert 转换后的处理函数
     * @return 转换后的分页对象
     */
    public static <T, U> Page<U> convertPage(Page<T> from, Function<T, U> func, Consumer<U> afterConvert) {
        if (ArrayUtil.isEmpty(from)) {
            return new Page<>();
        }
        Page<U> page = new Page<>(from.getCurrent(), from.getSize(), from.getTotal());
        List<U> records = from.getRecords().stream()
                .map(func)
